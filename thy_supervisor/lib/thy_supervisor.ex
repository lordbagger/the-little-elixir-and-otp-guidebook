defmodule ThySupervisor do
  ...
  def start_link(children_spec_list) do
    GenServer.start_link(__MODULE__, [children_spec_list])
  end
  ...
  def terminate_child(supervisor, pid) do
    GenServer.call(supervisor, {:terminate, pid})
  end
  ...
  def init([children_spec_list]) do
  ...
  def handle_call({:restart, old_pid, child_spec}, _from, state) do
  ...
  def handle_info({:EXIT, old_pid, _}, state) do
    case Map.fetch(state, old_pid) do
      {:ok, child_spec} ->
        case restart_child(old_pid, child_spec) do
          {:ok, pid} ->
  ...
  defp terminate_children(children_specs) do
    children_specs |> Enum.each(fn {pid, _} -> terminate_child(pid) end)
  end
